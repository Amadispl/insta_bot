...
class InstaPy:
    ...
    def open_photo(self):
        ...
        try:
            self.photo = self.driver.find_element(By.XPATH,'/html/body/div[2]/div/div/div[2]/div/div/div[1]/div[1]/div[2]/section/main/article/div/div/div/div[' + self.downIndex + ']/div[' + self.leftIndex + ']/a')
        except Exception:
            self.driver.refresh()
        self.photo.click()
        time.sleep(2)
    ...
    def like_photo_and_comment(self):
        ...
        self.x = 0
        if self.commentif > 80 and self.followif > 50:
            self.instabot['Likes'] += 1
            try:
                self.left_comment()
                print("Zdjęcie polaikowane, komentarz i follow")
                ...
                try:
                    ...
                    print("Zdjęcie polaikowane i komentarz, nie można zostawić followa")
                    self.instabot['Likes'] += 1
                    self.instabot['Comments'] += 1
            except:
                try:
                    ...
                    print("Zdjęcie polaikowane, nie można zostawić followa i komentarza")
                    self.instabot['Likes'] += 1
                print("Zdjęcie polaikowane i follow, nie można zostawić komentarza")
            ...
            try:
                ...
                print("Zdjęcie polaikowane, nie można zostawić komentarza")
                self.instabot['Likes'] += 1
        elif self.followif > 50:
            try:
                self.left_follow()
                self.instabot['Follows'] += 1
                self.instabot['Likes'] += 1
                print("Zdjęcie polaikowane i follow")
            except:
                print("Zdjęcie polaikowane, nie można zostawić followa")
                self.instabot['Likes'] += 1
    ...
    def left_follow(self):
        self.follow = self.driver.find_element(By.XPATH,'/html/body/div[6]/div[1]/div/div[3]/div/div/div/div/div[2]/div/article/div/div[2]/div/div/div[1]/div/header/div[2]/div[1]/div[2]/button')
        self.follow.click()
        time.sleep(2)
        self.instabot['Runtime'] += 2
    ...
    def relax(self):
        self.relax_time = random.randint(60, 90)
        self.y = 0
    ...
    def random_like(self):
        ...
        self.amount = random.randint(2, 5)  # Liczba polubień na jedną sesję.
    ...
    def auto_like(self):
        ...
        while True:
            if p_liked == self.amount:  # liczba przejść petli:
                ...
                self.relax_after_like = random.randint(1, 4)
                ...
                print(self.instabot)
                self.cursor.execute(self.update,self.instabot['Follows'], self.instabot['Comments'], self.instabot['Likes'], self.instabot['Runtime'],self.instabot['Name'])
                self.conn.commit()
                self.cursor.close()
                self.conn.close()
